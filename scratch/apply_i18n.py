...
css_replacement = """    /* Language Selector */
    #lang-selector {
...
# 2. Add Lang Selector to Nav
nav_target = '<div id="swk-button-wrapper"></div>'
nav_replacement = """<div style="display: flex; align-items: center;">
...
# 3. HTML attribute replacements (data-i18n)
replacements = {
    '<span>SHx Tip Bot</span>': '<span data-i18n="nav_title">SHx Tip Bot</span>',
    '<h1>Community Portal': '<h1><span data-i18n="hero_title">Community Portal</span>',
    '<p>Securely link your Discord and manage claims.</p>': '<p data-i18n="hero_desc">Securely link your Discord and manage claims.</p>',
    '<div id="connection-status-badge" class="badge badge-error mt-4">Wallet: Not Connected</div>': '<div id="connection-status-badge" class="badge badge-error mt-4" data-i18n="wallet_not_connected">Wallet: Not Connected</div>',
    
    '<div id="link-status-badge" class="badge badge-error text-xs mt-1">Status: Checking...</div>': '<div id="link-status-badge" class="badge badge-error text-xs mt-1" data-i18n="status_checking">Status: Checking...</div>',
    
    '<div class="stat-label">Discord Balance <div class="info-trigger">?</div>': '<div class="stat-label"><span data-i18n="stat_discord_bal">Discord Balance</span> <div class="info-trigger">?</div>',
    '<b>Internal Tipping Account</b><p class="text-xs text-muted mb-2">Move SHx from your wallet here for tipping:</p>': '<b data-i18n="tt_internal_acc">Internal Tipping Account</b><p class="text-xs text-muted mb-2" data-i18n="tt_desc">Move SHx from your wallet here for tipping:</p>',
    '<li>Type <b>/deposit</b> in Discord.</li>': '<li data-i18n="tt_li1">Type <b>/deposit</b> in Discord.</li>',
    '<li>Send SHx with Memo ID:': '<li data-i18n="tt_li2">Send SHx with Memo ID:',
    
    '<div class="stat-label">Wallet Balance</div>': '<div class="stat-label" data-i18n="stat_wallet_bal">Wallet Balance</div>',
    
    'onclick="resetSession()">Trouble connecting? Reset Session</p>': 'onclick="resetSession()" data-i18n="reset_session">Trouble connecting? Reset Session</p>',
    
    '<button id="btn-link" class="btn btn-primary w-full justify-center" disabled>Verify & Link Wallet</button>': '<button id="btn-link" class="btn btn-primary w-full justify-center" disabled data-i18n="btn_link">Verify & Link Wallet</button>',
    '<button id="btn-unlink-action" class="btn btn-secondary w-full justify-center">Unlink Current Wallet</button>': '<button id="btn-unlink-action" class="btn btn-secondary w-full justify-center" data-i18n="btn_unlink">Unlink Current Wallet</button>',
    
    '<h2>Pending Withdrawals</h2>': '<h2 data-i18n="pending_withdrawals">Pending Withdrawals</h2>',
    '<p class="text-sm text-muted mb-4">Tickets created from Discord. Select one to claim.</p>': '<p class="text-sm text-muted mb-4" data-i18n="tickets_desc">Tickets created from Discord. Select one to claim.</p>',
    '<div class="text-center py-4 text-muted">Scanning tickets...</div>': '<div class="text-center py-4 text-muted" data-i18n="scanning_tickets">Scanning tickets...</div>',
    
    '<h3 class="mb-2">Claiming:': '<h3 class="mb-2"><span data-i18n="claiming">Claiming</span>:',
    '<p class="mb-4">Amount:': '<p class="mb-4"><span data-i18n="amount">Amount</span>:',
    
    '<button id="btn-claim-action" class="btn btn-primary bg-success w-full justify-center" disabled>Confirm & Claim on Stellar</button>': '<button id="btn-claim-action" class="btn btn-primary bg-success w-full justify-center" disabled data-i18n="btn_claim">Confirm & Claim on Stellar</button>',
    '<button id="btn-claim-cancel" class="btn btn-danger w-full justify-center mt-4">Cancel Ticket & Refund to Discord</button>': '<button id="btn-claim-cancel" class="btn btn-danger w-full justify-center mt-4" data-i18n="btn_cancel">Cancel Ticket & Refund to Discord</button>',
    '<p class="text-xs text-center mt-4 text-muted" onclick="showList()" style="cursor:pointer; text-decoration: underline;">← Back to List</p>': '<p class="text-xs text-center mt-4 text-muted" onclick="showList()" style="cursor:pointer; text-decoration: underline;" data-i18n="back_list">← Back to List</p>',
    
    'Questions?: Email <a': '<span data-i18n="questions">Questions?: Email</span> <a'
}
...
# 4. Javascript Translations injection
js_injection = """
...
# Insert right after <script> block
script_target = "<script>"
content = content.replace(script_target, f"{script_target}\n{js_injection}")

# Fix javascript inline strings

js_replaces = {
    '<div class="text-center py-4 text-muted">Guest Mode: Use <b>/link</b> in Discord to see your tickets.</div>': '`<div class="text-center py-4 text-muted">${t(\'js_guest_mode\')}</div>`',
    '<div class="text-center py-4 text-muted">Wait for a fresh link from Discord...</div>': '`<div class="text-center py-4 text-muted">${t(\'js_wait_fresh\')}</div>`',
    '<div class="text-center py-4 text-muted">No pending tickets found.</div>': '`<div class="text-center py-4 text-muted">${t(\'js_no_tickets\')}</div>`',
    '<p class="text-xs text-muted">Ticket: ...${w.id.slice(-6)}</p>': '<p class="text-xs text-muted">${t(\'js_ticket\')} ...${w.id.slice(-6)}</p>',
    '<span onclick="handleCancel(\'${w.id}\')" class="text-error text-xs" style="cursor:pointer; text-decoration: underline; margin-top: 4px; display: inline-block;">Cancel & Refund</span>': '<span onclick="handleCancel(\'${w.id}\')" class="text-error text-xs" style="cursor:pointer; text-decoration: underline; margin-top: 4px; display: inline-block;">${t(\'js_cancel_refund\')}</span>',
    '<button onclick="selectTicket(\'${w.id}\', \'${w.amount}\')" class="btn btn-primary text-xs px-4 py-2">Select to Claim</button>': '<button onclick="selectTicket(\'${w.id}\', \'${w.amount}\')" class="btn btn-primary text-xs px-4 py-2">${t(\'js_select_claim\')}</button>',
    'notify(\'claim-notify\', "Review details and confirm below.");': 'notify(\'claim-notify\', t(\'js_review\'));',
    'if(confirm("Reset session?"))': 'if(confirm(t(\'js_reset_prompt\')))',
    'if (confirm("Unlink wallet?"))': 'if (confirm(t(\'js_unlink_prompt\')))',
    'hero.innerText = `Verified:': 'hero.innerText = `${t(\'js_verified\')}:',
    'profile.innerText = linked ? "Status: Linked ✅" : "Status: Wallet Not Linked";': 'profile.innerText = linked ? t(\'js_status_linked\') : t(\'js_status_not_linked\');',
    'linkBtn.innerText = linked ? "Linked ✅" : (existing && existing.length === 56 ? "Switch to this Wallet" : "Verify & Link Wallet");': 'linkBtn.innerText = linked ? t(\'js_status_linked\').replace("Status: ", "") : (existing && existing.length === 56 ? t(\'js_switch_wallet\') : t(\'btn_link\'));',
    'hero.innerText = "Wallet: Not Connected";': 'hero.innerText = t(\'wallet_not_connected\');',
    'fetchStellarBalance(address) {\\n        if (!address || !checkSdkReady()) return;': 'fetchStellarBalance(address) {\\n        if (!address || !checkSdkReady()) return;',
    '} catch (e) { document.getElementById(\'external-balance-val\').innerText = "Check Trustline"; }': '} catch (e) { document.getElementById(\'external-balance-val\').innerText = t(\'js_check_trustline\'); }',
    
    'return alert("Connect wallet first.");': 'return alert(t(\'js_connect_first\'));',
    'return alert("Guest Mode: Please navigate here using the /link command from Discord to verify your wallet.");': 'return alert(t(\'js_guest_alert\'));',
    'notify(\'link-notify\', "Awaiting signature...");': 'notify(\'link-notify\', t(\'js_awaiting_sig\'));',
    'notify(\'claim-notify\', "Preparing claim...");': 'notify(\'claim-notify\', t(\'js_preparing\'));',
    'notify(\'claim-notify\', "Confirming...");': 'notify(\'claim-notify\', t(\'js_confirming\'));',
    'notify(\'claim-notify\', `✅ Success! <a href="${txUrl}" target="_blank" style="color:var(--accent); text-decoration:underline;">View on Explorer</a>`);': 'notify(\'claim-notify\', `${t(\'js_success\')} <a href="${txUrl}" target="_blank" style="color:var(--accent); text-decoration:underline;">${t(\'js_view_explorer\')}</a>`);',
    'if (!confirm("Are you sure you want to cancel this withdrawal and refund the SHx back to your Discord balance?")) return;': 'if (!confirm(t(\'js_refund_prompt\'))) return;',
    'notify(\'claim-notify\', "Processing refund...");': 'notify(\'claim-notify\', t(\'js_processing\'));',
    'notify(\'claim-notify\', "✅ Refunded back to Discord!");': 'notify(\'claim-notify\', t(\'js_refunded\'));',
    'setStatus("Linked ✅");': 'setStatus(t("js_status_linked").replace("Status: ", ""));',
}

for target, repl in js_replaces.items():
    content = content.replace(target, repl)
